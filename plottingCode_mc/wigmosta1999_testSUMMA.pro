...
fpath = root + 'output/syntheticTestCases/wigmosta1999/'
...
for jTime=0,n_elements(outTime)-1 do begin
 ...
 for iExp=0,n_elements(cExp)-1 do begin
  ...
  for iHRU=0,nHRU-1 do begin
   ...
   ncdf_varget, nc_file, ivar_id, mLayerDepth, offset=[iHRU,0,itime], count=[1,nSoil[0,itime-1],1]

   ; get the desired variable for all layers
   ivar_id = ncdf_varid(nc_file,'mLayerVolFracLiq')
   ncdf_varget, nc_file, ivar_id, mLayerVolFracLiq, offset=[iHRU,0,itime], count=[1,nSoil[0,itime-1],1] 
